...
export const Login = () => {
  ...
  //   setFormulaireConnexion(!formulaireConnexion);
  // }
  // TODO : cree un fichier login.ts et importer la fonction login
  // Fonction appelée quand on clique sur "Se connecter"
  async function soumettreConnexion(evenement: any) {
    evenement.preventDefault();

    const reponseApi = await fetch("http://localhost/api/Controller/login.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: adresseMail, mot_de_passe: motDePasse })
    });

    const donneesConnexion = await reponseApi.json();
